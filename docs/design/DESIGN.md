...
### ADR-5. Agent와 Tool 실행 경계를 분리한다

Agent 기능은 백엔드의 `agent`, `tool`, `permission` 도메인에서 오케스트레이션한다. 백엔드는 LLM 호출, tool schema/registry, tool 요청 생성, 권한 정책, 감사 경계를 소유하고, 사용자 로컬 워크스페이스 파일 접근은 VS Code Extension Host의 로컬 tool executor registry가 수행한다. v1 tool calling은 `read_file`, `search_files` 같은 읽기 전용 tool만 자동 실행한다.
...
1. 사용자가 VS Code 확장에서 질문과 승인 모드(`ask`, `approve`, `full`)를 전송한다.
2. 확장은 `/agent/stream`으로 agent 요청을 보내고 SSE 스트림을 연다.
3. `AgentService`가 기존 프롬프트 조립 결과에 `ToolPromptService`가 registry/schema에서 렌더링한 tool calling 지침과 권한 모드를 추가해 중앙 Ollama에 요청한다.
4. 모델이 `<klee_tool_call>{...}</klee_tool_call>` 형식으로 읽기 전용 tool을 요청하면 `ToolExecutorService`가 정책을 확인하고 백엔드는 `tool_call_requested` SSE 이벤트를 보낸다.
5. Extension Host가 로컬 tool executor registry에서 tool을 찾아 워크스페이스 내부에서만 실행하고 `/agent/tool-results`로 결과를 반환한다.
6. 백엔드는 tool observation을 다시 모델 프롬프트에 붙여 최종 답변을 생성하고 `token`, `done` 이벤트로 반환한다.
