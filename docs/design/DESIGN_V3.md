...
Slash command는 두 종류로 나눈다.

- Extension-local command: `extension/src/extension-host/chat/slashCommand.ts`의 `LOCAL_SLASH_COMMANDS`에서 관리한다. 모델 호출 전에 extension host가 처리한다.
- Prompt skill command: local command에 해당하지 않는 `/스킬명`은 백엔드 `skillCommand`로 전달하고, 백엔드가 내부 스킬 또는 `.klee/skills`를 프롬프트에 합성한다.
